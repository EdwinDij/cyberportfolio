...
const GlitchText = ({ text, className }: GlitchTextProps) => {
  ...
  return (
    <div className={`relative ${className}`}>
      <span className="absolute inset-0 text-primary opacity-80" style={{ clipPath: glitch ? `inset(${Math.random() * 100}% 0 ${Math.random() * 100}% 0)` : 'inset(50% 0 50% 0)' }}>{text}</span>
      {text}
      <span className="absolute inset-0 text-secondary opacity-80" style={{ clipPath: glitch ? `inset(${Math.random() * 100}% 0 ${Math.random() * 100}% 0)` : 'inset(50% 0 50% 0)' }}>{text}</span>
    </div>
  );
};
...
export const CyberLoader = () => {
  ...
  useEffect(() => {
    if (step < lines.length - 1) {
      const timer = setTimeout(() => setStep(s => s + 1), step < 4 ? 300 : 500);
      ...
      <AnimatePresence>
        {step >= 7 && (
          ...
          >
            ...
            <GlitchText text="! SYSTEM BREACH !" className="text-4xl font-heading mt-4" />
          </motion.div>
        )}
      </AnimatePresence>
